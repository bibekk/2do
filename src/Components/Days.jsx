          ...
          return(
            tasks.filter(f=> new Date(f.duedate).toDateString() === new Date(`${dayjs().add(_day,'day')}`).toDateString() && f.completed === 0).length > 0 &&
              <div key={_day} className={_day !== 0 ? "space-y-1 bg-gray-500 rounded-lg p-2 max-w-lg":"space-y-1 bg-red-400 rounded-lg p-2 max-w-lg"}>     
                <span className='task-date'>{`${dayjs().add(_day,'day').format('MMM D,YYYY')}`}</span><span className="text-blue-300 text-sm ml-1">{`${dayjs().add(_day,'day').format('dddd')}`}</span>
                    ...
                    <div className={`task ${m.completed === 1 ?'bg-green-100!':null}`} key={m.task_id}>
                      <div className='flex justify-between'>               
                        <span  className='flex flex-grow'>
                          {m.completed === 1? <FaCircleCheck className=' mr-2 mt-1 text-green-500 hover:text-blue-500 hover:cursor-pointer'/>:<FaCircle className='float-left mr-2 mt-1 text-lg text-white hover: cursor-pointer hover:text-blue-500' onClick={async()=>await completeTask({task_id: m.task_id, status:m.completed})} />}
                      ...
                      { expanded.indexOf(m.task_id) > 0 && //m.expanded
                        <>
                          <div className='task-note'>{m.note.length > 2 ? m.note:<div className="message_notfound">No Note</div>}</div>
