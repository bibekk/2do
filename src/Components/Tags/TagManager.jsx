...
const TagManager = ({ clearDataCallback}) => {
  ...
  return (
    <Modal clearDataCallback={clearDataCallback} title={"Tags"} width={50} >
      ...
      {tags.map((m,i) => {
          ...
          ({taskstags.filter(f=> f.tag_id === m.tag_id).length})
            <div className="flex gap-1 mt-1  ml-2">
              ...
              <MdDelete className='text-red-400 text-sm hover:text-red-700 hover:cursor-pointer' onClick={()=>{if(window.confirm('Are you sure you want to delete?')) {dispatch(deleteTag(m.tag_id))}}}/>
            </div>
          </div>)
      })}
      ...
      {showEditTag.show && 
        <EditTag clearDataCallback={()=> setShowEditTag({show: false, tag: null})} tag={showEditTag.tag} />
      }
      </div>
    </Modal>
  )
