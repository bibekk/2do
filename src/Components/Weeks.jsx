...
const ThreeWeeks = ({ setShowEditTask}) => {
                  ...
                  <div className={`task ${m.completed === 1 ?'bg-green-100!':null}`} key={m.task_id}>
                    <div className='flex justify-between'>               
                      <span  className='flex flex-grow'>
                        {m.completed === 1? <FaCircleCheck className=' mr-2 mt-1 text-green-500 hover:text-blue-500 hover:cursor-pointer' />:<FaCircle className='float-left mr-2 mt-1 text-lg text-white hover: cursor-pointer hover:text-blue-500' onClick={()=>completeTask({task_id: m.task_id, status: m.completed})} />}
